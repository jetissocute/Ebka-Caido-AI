...
export const startToolExecution = (
  sessionId: number,
  toolName: string,
  toolInput: any,
) => {
  ...
  console.log(
    `[BACKEND] startToolExecution: Set state for session ${sessionId}:`,
    state,
  );
};
...
export const stopToolExecution = (sessionId: number) => {
  ...
  console.log(
    `[BACKEND] stopToolExecution: Removed state for session ${sessionId}, was executing: ${wasExecuting}`,
  );
};
...
export const getToolExecutionState = (sessionId: number) => {
  ...
  console.log(
    `[BACKEND] getToolExecutionState called for session ${sessionId}, state:`,
    state,
  );
  return state;
};

export const saveToolExecutionState = (
  sessionId: number,
  toolName: string,
  toolInput: any,
) => {
