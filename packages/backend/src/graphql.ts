...
): Promise<{ success: boolean; data?: T; error?: string; response?: GraphQLResponse<T> }> => {
  try {
    ...
    if (!authToken) {
      return {
        success: false,
        error: "No Caido auth token found. Please set the auth token first using request-auth-token event."
      };
    }
